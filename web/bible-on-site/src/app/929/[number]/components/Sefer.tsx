...
const hePageSemantics: PageSemantics = {
  ...
  semanticNameToIndex(semanticPageName: string): number | null {
    const entry = Object.entries(hePageSemanticsDict).find(([, value]) => value === semanticPageName);
...
const Sefer = (props: { perekObj: PerekObj }) => {
        ...
        <section className={styles.page}>
          <article className={styles.perekText}>
            {perek.pesukim.map((pasuk, pasukIdx) => {
              ...
              const pasukNumElement = <a className={styles.pasukNum}>{toLetters(pasukIdx + 1)}</a>;
  ...
  return <FlipBook className="he-book" pages={pages} pageSemantics={hePageSemantics} direction="rtl" />;
};

...
CREATE TABLE parshan (
  ...
  name TEXT NOT NULL UNIQUE,   -- Primary Hebrew name of the commentator
  birth_year INTEGER,          -- When known (e.g. from Sefaria person)
...
CREATE TABLE perush (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL UNIQUE,   -- Hebrew display name (e.g., רש"י, אבן עזרא)
  parshan_id INTEGER NOT NULL,
  comp_date TEXT,              -- Composition date (as-is from Sefaria)
  pub_date TEXT,               -- Publication date
  priority INTEGER NOT NULL DEFAULT 0,  -- Display order (lower = first)

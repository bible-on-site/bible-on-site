...
mod tests {
    ...
    fn bson_to_shell_format_formats_nested_documents_with_unquoted_keys() {
        ...
        assert!(shell.contains("$match: {"));
        assert!(shell.contains("title: \"Rashi\""));
        assert!(shell.contains("enabled: true"));
        assert!(shell.contains("count: 2"));
    }
    ...
    fn compass_stage_filename_uses_named_stage_and_fallback() {
        let stage_names = ["match-index", "lookup-texts"];

        assert_eq!(
            compass_stage_filename(0, &stage_names),
            "01-match-index.mongodb-compass-stage"
        );
        assert_eq!(
    ...
    fn compass_stage_content_formats_document_stage_with_header() {
        let stage = doc! {
            "$match": {
                "collective_title": "Rashi",
                "versions": "$version_titles",
            }
        };

        let content = compass_stage_content(1, &stage);

        assert!(content.starts_with("// Stage 2: $match\n"));
        assert!(content.contains("collective_title: \"Rashi\""));
        assert!(content.contains("versions: \"$version_titles\""));
    }
